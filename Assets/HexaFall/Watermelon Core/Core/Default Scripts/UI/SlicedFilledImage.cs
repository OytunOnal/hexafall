...
#pragma warning restore 1692
	...
	{
		public static TweenCase DOFillAmount(this SlicedFilledImage tweenObject, float resultValue, float time, float delay = 0, bool unscaledTime = false, TweenType tweenType = TweenType.Update)
		{
			return new TweenCaseSlicedImageFill(tweenObject, resultValue).SetDelay(delay).SetTime(time).SetUnscaledMode(unscaledTime).SetType(tweenType).StartTween();
		}
